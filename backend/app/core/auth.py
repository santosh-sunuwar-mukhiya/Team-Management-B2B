...
def require_view(user: Annotated[AuthUser, Depends(get_current_user)]) -> AuthUser:
...
def require_create(user: Annotated[AuthUser, Depends(get_current_user)]) -> AuthUser:
    if not user.can_create:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Create permission required"
        )

    return user


def require_delete(user: Annotated[AuthUser, Depends(get_current_user)]) -> AuthUser:
    if not user.can_delete:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Delete permission required"
        )

    return user


def require_edit(user: Annotated[AuthUser, Depends(get_current_user)]) -> AuthUser:
    if not user.can_edit:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Edit permission required"
        )
